...
def registerNew():
    root= Tk()
    root.title("Register New (Camera)")
...
# def faceRecognition ():
    if running:
        ...
        @app.route("/shutdown")
        def shutdown():
            Popen('python gui.py', shell='False') # in windows use this line
            # Popen('python3 gui.py', shell='False') # in linux/raspberry pi use this line
            pid = os.getpid()
            print('Back to main GUI...')
            os.kill(int(pid), signal.SIGINT) # in windows, use this line
